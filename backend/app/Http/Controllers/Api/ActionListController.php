...
class ActionListController extends Controller
{
    ...
    public function updateAction(Request $request, int $actionListId, int $actionId): JsonResponse
    {
        ...
        $validated = $request->validate([
            'type' => 'required|string|in:email,webhook,change_setting,remove_attendees_from_reservation_list,remove_attendees_from_waitlist,remove_mail_validation_ip_rate_limits,archive_reservation_and_waiting_list,wait_n_seconds,change_default_guest_token,data_portability_backup,data_portability_transport',
